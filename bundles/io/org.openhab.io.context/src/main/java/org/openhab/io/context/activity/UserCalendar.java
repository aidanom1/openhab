...
public class UserCalendar {
  ...
  private static Credential authorize() throws Exception {
    ...
    GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(JSON_FACTORY,
        new InputStreamReader(UserCalendar.class.getResourceAsStream("/client_secrets.json")));
  ...
  private static void showCalendars() throws IOException {
	  logger.debug("Show Calendars");
    CalendarList feed = client.calendarList().list().execute();
    logger.debug(feed.toPrettyString());
  }

  private static void addCalendarsUsingBatch() throws IOException {
	  logger.debug("Add Calendars using Batch");
    BatchRequest batch = client.batch();

    // Create the callback.
    JsonBatchCallback<Calendar> callback = new JsonBatchCallback<Calendar>() {

      public void onSuccess(Calendar calendar, HttpHeaders responseHeaders) throws IOException {
    	  logger.debug(calendar.toPrettyString());
  ...
  private static void deleteCalendarsUsingBatch() throws IOException {
	  logger.debug("Delete Calendars Using Batch");
    BatchRequest batch = client.batch();
